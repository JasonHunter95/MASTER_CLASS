...
```{r loading data}
load("./data/count_files.RData", verbose = TRUE)
load("./data/deseq_samples.RData", verbose = TRUE)
```
...
# we need to merge g2s and res_0_df
# with a column that exists in both DFs = gene_id
...
view(res_0_df)
# Let's organize this so the gene name is next to gene_id
...
```
# congratulations you have started your first RNAseq analyses !
