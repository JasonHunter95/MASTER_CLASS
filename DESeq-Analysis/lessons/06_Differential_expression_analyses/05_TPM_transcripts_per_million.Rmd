...
# TPM <- read.table("/scratch/Shares/rinnclass/MASTER_CLASS/STUDENTS/jahu8701/MASTER_CLASS/lessons/04_RNAseq_Dox/01_Mouse_dox_wt/dox_long/job_output/star_salmon/salmon.merged.gene_tpm.tsv", header=TRUE, row.names=1) # nolint
TPM <- read.table("data/salmon.merged.gene_tpm.tsv", header=TRUE, row.names=1) # nolint

view(TPM)
# Ok so we can see that we are missing our gene_names
# but have gene_id and a TPM value for each gene across each timepoint
...
# We are going to do this by indexing into TPM
# and grabbing all rows that add up to 0
TPM_zero <- TPM[rowSums(TPM == 0) == ncol(TPM), ] # nolint
view(TPM_zero)
...
# first lets move the gene_id into a column
# to cross compare between TPM and TPM_zero
view(TPM)
TPM <- rownames_to_column(TPM, "gene_id") # nolint
view(TPM)
# same for TPM_zero
TPM_zero <- rownames_to_column(TPM_zero, "gene_id") # nolint

# Now we can remove these from TPM using the Tidyverse %in% 'function'
TPM_not_zero <- TPM[!(TPM$gene_id %in% TPM_zero$gene_id), ] # nolint
view(TPM_not_zero)
...
# weird we still see a lot of small values (e.g., 0.2314).
# let's filter our TPM object differently

# First some cleaning up
# Moving gene_id column back to rownames
view(TPM)
row.names(TPM) <- TPM$gene_id # nolint
view(TPM)

# now removing the actual gene_id col
TPM$gene_id <- NULL # nolint
view(TPM)

# last clean up to move rownames back
TPM <- column_to_rownames(TPM, "gene_id") # nolint
view(TPM)
...
for (tp in time_points) {
  ...
  X_average_values <- c(X_average_values, list(avg)) # nolint
}

# Convert the list to a data frame
X_average_values <- do.call(cbind, X_average_values) # nolint

# Add column names for the time points
colnames(X_average_values) <- time_points # nolint

# convert back to data.frame
avrg_TPM <- as.data.frame(X_average_values) # nolint
